...
class MainActivity : AppCompatActivity() {
        ...
        return when(item.itemId) {
            ...
            R.id.callMi -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    // Permissão já foi dada durante a instalação pq o Android é menor que M
                    chamarNumero(true)
                }
    ...
    private fun chamarNumero(chamar: Boolean) {
        ...
        val intentMuitoLoca = Intent(if(chamar) ACTION_CALL else ACTION_DIAL)
        intentMuitoLoca.data = numeroUri
        startActivity(intentMuitoLoca)
    }
}
